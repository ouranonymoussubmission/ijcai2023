...
def load_data(args, debug=True):
    root = "/home/anonymous"
    train_data = pd.read_csv(f"{root}/ijcai2023/data/fold{args.fold}_london_aq_train.csv")
    if args.mode == "test":
        test_data = pd.read_csv(f"{root}/ijcai2023/data/fold{args.fold}_london_aq_test.csv")
    ...
    X_train, y_train, scaler = preprocess(train_data)
    if args.mode == "test":
        X_test, y_test, _ = preprocess(test_data, scaler)
    else:
...
def test(args):
    ...
    print(args.features)
    # os.makedirs(join(args.root, f"fet-{'_'.join(sorted(args.features))}.pkl"))
